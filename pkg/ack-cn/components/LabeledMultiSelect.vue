...
export default {
  ...
  props: {
    ...
    hoverTooltip:   { default: true, type: Boolean },
    loading:        { default: false, type: Boolean },
    localizedLabel: { default: false, type: Boolean },
    optionKey:      { default: null, type: String },
    optionLabel:    { default: 'label', type: String },
    placement:      { default: null, type: String },
    reduce:         {
      default: (e) => (e && typeof e === 'object' && e.value !== undefined ? e.value : e),
      type:    Function,
    },
    selectable: {
      default: (opt) => opt && !(opt.disabled || LABEL_SELECT_NOT_OPTION_KINDS.includes(opt.kind) || opt.loading),
      type:    Function,
    },
  ...
  methods: {
    focusSearch() {
      ...
      if (!this.focused && blurredAgo < 250) return;
    ...
    getOptionLabel(option) {
      if (!option) return;
      if (this.$attrs['get-option-label']) return this.$attrs['get-option-label'](option);
      const label = get(option, this.optionLabel);

      return label ? (this.localizedLabel ? this.$store.getters['i18n/t'](label) || label : label) : option;
    },
    ...
    getOptionKey(opt) {
      return this.optionKey ? get(opt, this.optionKey) : this.getOptionLabel(opt);
    },
  },
};
</script>
...
<template>
  <div
    ref="select"
    class="labeled-multi-select labeled-select"
    :class="[
      $attrs.class,
      {
...
.vs__dropdown-menu {
  .vs__dropdown-option .vs__option-kind {
    ...
    &.has-icon { padding-left: $icon-size; }
  }

  &.has-icon .vs__option-kind div { padding-left: $icon-size; }
...
.vs__dropdown-option {
  > .option-kind-highlighted {
    ...
    &:hover { color: var(--dropdown-hover-text); }
  }
  &.vs__dropdown-option--selected,
  &.vs__dropdown-option--highlight {
    > .option-kind-highlighted { color: var(--dropdown-hover-text); }
  }
}
</style>
