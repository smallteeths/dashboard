...
export const DISPLAY_NAME_ANNO = 'provisioning.cattle.io/management-cluster-display-name';
...
export async function loadClusterInstallData(store, clusterId, t) {
  ...
  if (!clusterId) {
    ...
    return {
      clusterCIDR: '', serviceCIDR: '', submarinerBrokerUrl: '', submarinerPsk: '', submarinerToken: '', submarinerClusterID: '', cacerts: '', warnings
    };
  }
  ...
  const [provClusters, cacertsSetting, serverUrlSetting] = await Promise.all([
    ...
    store.dispatch('management/find', { type: MANAGEMENT.SETTING, id: 'cacerts' }).catch((err) => {
      warnings.push(`${ t('mcs.errors.fetchCacertsFail') }:${ err?.message }`);

      return { value: '' };
    }),
    store.dispatch('management/find', { type: MANAGEMENT.SETTING, id: 'server-url' }).catch((err) => {
      warnings.push(`${ t('mcs.errors.fetchServerUrlFail') }:${ err?.message }`);
  ...
  if (provCluster?.metadata?.annotations) {
    if (provCluster?.metadata?.annotations['service-cidr'] && !mgc['service-cidr']) {
      mgc['service-cidr'] = provCluster?.metadata?.annotations['service-cidr'];
    }
    if (provCluster?.metadata?.annotations['cluster-cidr'] && !mgc['cluster-cidr']) {
      mgc['cluster-cidr'] = provCluster?.metadata?.annotations['cluster-cidr'];
    }
  }
  ...
  const [brokerData, clusterSets] = await Promise.all([
    provName ? store.dispatch('management/request', { url: `/mcs-api/registration?clusterName=${ provName }` })
      .catch((err) => {
        warnings.push(`${ t('mcs.errors.brokerRegisterFail') }:${ err.message }`);

        return null;
      }) : Promise.resolve(null),
    canListClusterSet ? store.dispatch('management/findAll', { type: CLUSTER_SET }).catch(() => []) : Promise.resolve([]),
  ]);
...
export async function upgradeMcsExtChart({
  ...
  try {
    ...
    try {
      const brokerData = await req(`/mcs-api/registration?clusterName=${ currentCluster?.metadata?.name }`);
