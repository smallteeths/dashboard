  ...
  <div v-else>
    ...
    >
      ...
      <template #cell:cluster="{row}">
        ...
        <template v-else>
          <span v-if="row.cluster">{{ row.cluster?.metadata?.annotations?.[displayNameAnno] || row.cluster.nameDisplay }}</span>
          <span
            v-else
            class="text-muted"
          >
      ...
      <template
        #row-actions="{row}"
        @click="showClusterAction($event, row)"
      >
        <ButtonMultiAction
          class="clusterset-action mr-10"
          :borderless="true"
...
const genRowObj = (cluster, clusterset, that, clusterName) => {
  // if cluster is undefined and have clusterName
  // the cluster is deleted
  let deletedClusterName = '';

  if (typeof cluster === 'undefined' && clusterName) {
    deletedClusterName = clusterName;
  }
  const actions = [];
  ...
  const d = {
    ...
    cluster:           (!cluster && deletedClusterName) ? { nameDisplay: deletedClusterName } : cluster,
    clusterset,
    creationTimestamp: clusterset ? clusterset.metadata.creationTimestamp : cluster?.metadata?.creationTimestamp,
    mainRowKey:        id,
    groupByLabel:      clusterset?.metadata?.name,
    ...
    remove() {
      that.$store.dispatch('management/promptModal', {
        ...
        componentProps: {
          ...
          clusterset,
          deletedClusterName,
          body:      `${ that.t('promptRemove.attemptingToRemove', { type: 'clusterset member' }) } <span>${ cluster?.metadata?.annotations[DISPLAY_NAME_ANNO] || cluster?.nameDisplay || deletedClusterName }</span>`,
        },
        modalWidth: '600px'
      });
    },
...
export default {
  ...
  data() {
    return {
      ...
      CLUSTER_SET,
      rancherClusters:   [],
      displayNameAnno:   DISPLAY_NAME_ANNO,
    };
  },
  async fetch() {
  ...
  computed: {
    ...
    filteredRows() {
      ...
      const rows = rowData.filter((r) => Object.keys(r.spec?.clusters ?? {}).length > 0).reduce((t, c) => {
        const rows = Object.keys(c.spec.clusters).map((item) => genRowObj(nameToClusterMap[item], c, this, item));
