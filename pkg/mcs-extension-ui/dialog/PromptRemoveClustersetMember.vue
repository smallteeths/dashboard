...
export default {
  ...
  async fetch() {
    const url = `k8s/clusters/${ this.cluster.status.clusterName }/v1/multicluster.x-k8s.io.serviceexports`;
  ...
  methods: {
    ...
    async confirm(result) {
      ...
      // the cluster is deleted and the clusterset has the cluster
      if (this.deletedClusterName && !this.cluster) {
        const sc = await this.$store.dispatch(`management/clone`, { resource: this.clusterset });

        delete sc.spec.clusters[this.deletedClusterName];
        await sc.save();
      ...
      try {
        ...
        try {
          ...
          if (error && error.Code?.Code !== 'ClusterUnavailable') {
            throw new Error(`Get apps errors：${ error?.Message || error._statusText || error }`);
          }
        }
        ...
        const c = await this.$store.dispatch('management/find', {
          type: CAPI.RANCHER_CLUSTER, id: this.cluster.id, opt: { force: true }
        } );
    ...
    <template #body>
      <slot name="body">
        <Banner
          v-if="serviceExports"
          color="warning"
          :label="t('mcs.moveClustersetMemeberModal.deleteNotice', { serviceExports })"
        />
        <div
          v-clean-html="decodeHtml(body)"
          class="pl-10 pr-10"
          style="min-height: 30px; display: flex;"
        />
      </slot>
    </template>
    <template #actions>
