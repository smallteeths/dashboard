...
export default {
  ...
  computed: {
    ...mapGetters({ currentCluster: 'currentCluster', theme: 'prefs/theme' }),
    dateRanges() {
      const arr = [{
  ...
  methods: {
    ...
    async loadLogs() {
      this.loading = true;
      try {
        const queryForm = { ...this.queryForm, clusterID: this.currentCluster.id };
  ...
  <div class="global-audit-log">
    ...
    >
      ...
      >
        <template #default="{ row }">
          <span
            v-if="row.requestResName"
            class="cell-label"
          > {{ row.requestResName }} </span><span v-else> {{ row.requestResId }} </span>
        </template>
      </VxeColumn>
      <VxeColumn
        field="detail"
