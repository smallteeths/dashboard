...
const hadInitialValue = ref(false);
...
watch(
  ...
  (val) => {
    ...
    hadInitialValue.value = Array.isArray(val) && val.length > 0;
    initialChecked.value = true;
  },
  { immediate: true }
);

// 只关心 options 的“内容变化”，不关心数组引用变化
const optionSignature = computed(() => {
  return (props.options || []).map((o) => o.value).join('|');
});

watch(
  [() => props.isNewOrUnprovisioned, optionSignature],
  ([isNewOrUnprovisioned]) => {
    // 不自动处理
    if (!isNewOrUnprovisioned) {
      return;
    }

    // 初始就有值：视为编辑态
    // 配合 isNewOrUnprovisioned 双重保险
    if (hadInitialValue.value) {
      return;
    }
