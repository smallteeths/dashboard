...
export default {
  ...
  methods: {
    ...
    addGlobalValuesTo(values) {
      ...
      const cluster = this.currentCluster;
      const isImported = cluster?.providerForEmberParam === 'import';
      const clusterDefaultRegistry = isImported ? cluster?.spec?.systemDefaultRegistry : '';
      const projects = this.$store.getters['management/all'](MANAGEMENT.PROJECT);
      const systemProjectId = projects.find(p => p.spec?.displayName === 'System')?.id?.split('/')?.[1] || '';
      const defaultRegistry = clusterDefaultRegistry || this.defaultRegistrySetting?.value || '';
