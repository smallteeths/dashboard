...
export default defineComponent({
  ...
  computed: {
    ...
    headers() {
      ...
      const cpu = headers.find((item) => item.name === 'cpu');

      if (cpu) {
        cpu.getValue = (row) => row.cpuUsagePercentage;
        cpu.formatter = 'CpuUsage';
      }
      const ram = headers.find((item) => item.name === 'ram');

      if (ram) {
        ram.getValue = (row) => row.ramUsagePercentage;
